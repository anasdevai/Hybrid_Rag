...
class FederatedRetriever:
    ...
    def _retrieve_section(self, section: str, query: str) -> List[Document]:
        """Run hybrid retrieval for one section (blocking, safe for threads)."""
        try:
            return self.retrievers[section].invoke(query)
        except Exception as e:
            print(f"  [federated] Warning: retrieval failed for '{section}': {e}")
            return []

    def search(self, query: str) -> Dict[str, List[Document]]:
        """
        ...
        async def _gather():
            tasks = [
                asyncio.to_thread(self._retrieve_section, section, query)
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    raw_results = list(pool.map(
                        lambda s: self._retrieve_section(s, query),
                        sections
                    ))
            else:
                raw_results = loop.run_until_complete(_gather())
        except RuntimeError:
            raw_results = [self._retrieve_section(s, query) for s in sections]
