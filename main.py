...
from routers import auth, chat_history, webhooks
...
async def smart_query_endpoint(req: QueryRequest, current_user: User = Depends(get_current_user)):
    """
    Smart multi-collection query with automatic routing.
    - Routes the query to only the relevant collection(s)
    - Returns: clean prose answer | citations | dynamic suggestions
    """
    if not req.query.strip():
        raise HTTPException(400, "Empty query")

    result = app.state.smart_rag.invoke(req.query)
...
async def federated_query_endpoint(req: QueryRequest, current_user: User = Depends(get_current_user)):
    if not req.query.strip():
        raise HTTPException(400, "Empty query")
    result = app.state.smart_rag.invoke(req.query)
    return SmartQueryResponse(
