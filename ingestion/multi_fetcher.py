...
async def fetch_all_entities() -> dict[str, List[Document]]:
    ...
    result = {
        "sops":       [d for item in sops_raw      if (d := _clean_sop(item))       is not None],
        "deviations": [d for item in devs_raw       if (d := _clean_deviation(item)) is not None],
        "capas":      [d for item in capas_raw      if (d := _clean_capa(item))      is not None],
        "decisions":  [d for item in decisions_raw  if (d := _clean_decision(item))  is not None],
        "audits":     [d for item in audits_raw     if (d := _clean_audit(item))     is not None],
    }
